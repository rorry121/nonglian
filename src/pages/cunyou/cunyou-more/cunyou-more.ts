...
/**
 * Generated class for the CunyouMorePage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-cunyou-more',
  templateUrl: 'cunyou-more.html',
})
export class CunyouMorePage {
  dataList:any=[];
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad CunyouMorePage');
    this.loadList();
  }
  loadList(){
    let loading=this.commonProvider.showLoading();
    let url='classes/CunyouList';
    let search='?where={"geoPoint":{"$nearSphere": {"__type": "GeoPoint","latitude": '+this.lnglatXY[1]+',"longitude": '+this.lnglatXY[0]+'}}}';
    url=url+search;
